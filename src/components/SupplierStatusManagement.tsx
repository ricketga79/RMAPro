...
export const SupplierStatusManagement = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setFormData({ name: '', color: 'azul', description: '' });
      setEditingId(null);
      fetchStatuses();
    }
  };
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Tem a certeza que deseja eliminar o estado "${name}"?`)) return;
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Estados RMA Fornecedor"
        description="Configure os diferentes passos do processo de RMA com fornecedores."
        action={
          <button 
            onClick={() => { setEditingId(null); setFormData({ name: '', color: 'azul', description: '' }); setIsModalOpen(true); }}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-orange-600 text-white font-bold rounded-lg hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20 text-sm"
          >
            <Plus size={18} />
            Novo Estado
          </button>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Estados de Fornecedor</h2>
        </div>
            ...
            <form onSubmit={handleSave} className="p-4 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="Ex: Pendente de Envio, Enviado..."
                />
              </div>
                  ...
                  {COLOR_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`h-10 rounded-lg flex items-center justify-center transition-all ${
                        formData.color === opt.value ? 'ring-2 ring-orange-500 ring-offset-2 dark:ring-offset-slate-900' : 'opacity-70 hover:opacity-100'
                      } ${opt.class}`}
                      title={opt.name}
                    >
              ...
              <div className="pt-4 flex justify-end gap-2">
                ...
                <button 
                  ...
                  className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg shadow-md transition-colors disabled:opacity-70"
                >
